...
export class FlightPlanUtils {
  ...
  public static addProcedure(
    ...
    (decodeResult.raw.procedures as Array<{ type: string; route: object }>).push({ type: type, route: route });
...
function addCompanyRoute(decodeResult: DecodeResult, value: string) {
  ...
  decodeResult.formatted.items.push({
    ...
    code: 'CR',
    label: 'Company Route',
    value: RouteUtils.routeToString(decodeResult.raw.company_route as import("../types/route").Route),
  });
}
